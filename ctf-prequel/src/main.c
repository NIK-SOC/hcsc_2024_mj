...
void print_debug_flag(int limit) {
    sqlite3 *db;
    sqlite3_stmt *stmt;
    int rc;
    char *sql = "SELECT flag FROM flag LIMIT ?;";
    printf("Fetching debug flags...\n");

    rc = sqlite3_open("messages_debug.db", &db);
    if (rc != SQLITE_OK) {
        fprintf(stderr, "Cannot open database: %s\n", sqlite3_errmsg(db));
        sqlite3_close(db);
    }
